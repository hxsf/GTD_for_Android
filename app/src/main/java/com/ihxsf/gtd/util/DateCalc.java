...
public class DateCalc {
    ...
    public static String fromToday(Date date) {
        ...
        if (ago > 0) {
            ...
            if (ago <= ONE_HOUR)
                ...
                return "明天" + calendar.get(Calendar.HOUR_OF_DAY) + "点"
                        + calendar.get(Calendar.MINUTE) + "分";
            else if (ago <= ONE_DAY * 3)
                return "后天" + calendar.get(Calendar.HOUR_OF_DAY) + "点"
                        + calendar.get(Calendar.MINUTE) + "分";
            else if (ago <= ONE_MONTH) {
                long day = ago / ONE_DAY;
                return day + "天后" + calendar.get(Calendar.HOUR_OF_DAY) + "点"
                        + calendar.get(Calendar.MINUTE) + "分";
            } else if (ago <= ONE_YEAR) {
                long month = ago / ONE_MONTH;
                long day = ago % ONE_MONTH / ONE_DAY;
                return month + "个月" + day + "天后"
                        + calendar.get(Calendar.HOUR_OF_DAY) + "点"
                        + calendar.get(Calendar.MINUTE) + "分";
            } else {
                long year = ago / ONE_YEAR;
                int month = calendar.get(Calendar.MONTH) + 1;// JANUARY which is 0 so month+1
                return year + "年后" + month + "月" + calendar.get(Calendar.DATE)
                        + "日";
            }
        }
    ...
    public static void main(String[] args) throws ParseException {
        ...
        System.out.println(DateCalc.fromDeadline(date));

        String before = "2012-12-12 0:0:59";
        date = simple.parse(before);
        System.out.println(DateCalc.fromToday(date));

        System.out.println(DateCalc.getFullDate());
        System.out.println(DateCalc.getDate());
    }
